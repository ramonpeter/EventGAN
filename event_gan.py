...
class EventGAN():
    def __init__(
        ...
        mmd_weight: float,
        mmd_kernel: Union[str, callable],
        mmd_kernel_widths: List[float],
        ...
        **kwargs
        ...
        self.mmd_kernel_widths = mmd_kernel_widths
        ...
        # Define networks
        self.generator = self.get_generator(n_particles, g_units, g_layers)
        self.discriminator = self.get_discriminator(n_particles, d_units, d_layers)
    ...
    def get_generator(
        ...
        """
        # Input
        noise = tf.keras.layers.Input(shape=(3*n_particles,), name='noise')
        masses = tf.keras.layers.Input(shape=(n_particles,), name='masses')

        # hidden layers
        x = noise
        for _ in range(n_layers):
            x = tf.keras.layers.Dense(n_units, activation=tf.keras.layers.LeakyReLU())(x)

        # 3-momenta
        three_momenta = tf.keras.layers.Dense(3 * n_particles, name="3-momenta")(x)
        mass_combined_momenta = tf.keras.layers.concatenate([masses, three_momenta], name="mass_and_momenta")

        #Get 4-momenta
        four_momenta = LorentzVector(n_particles=n_particles, name="4-momenta")(mass_combined_momenta)

        return tf.keras.Model([noise, masses], four_momenta, name=name)
    ...
    def get_discriminator(
        ...
        """
        # Input
        events = tf.keras.layers.Input(shape=(4*n_particles,), name='events')
        ...
        return tf.keras.Model(events, logit, name=name)

    def save_weights(self, suffix: str=''):
        """Save the model weights"""
        self.generator.save_weights(f"{self.save_path}/generator_weights{suffix}.h5", save_format='h5')
        self.discriminator.save_weights(f"{self.save_path}/discriminator_weights{suffix}.h5", save_format='h5')


    def load_weights(self, suffix: str=''):
        """Load the model weights"""
        self.generator.load_weights(f"{self.save_path}/generator_weights{suffix}.h5")
        self.discriminator.load_weights(f"{self.save_path}/discriminator_weights{suffix}.h5")
    ...
    def initialize_data(
        ...
        assert 4*masses.shape[1] == train_data.shape[1], "Masses do not match input data"
    ...
    def train_step(self, 
        ...
        with tf.GradientTape() as tape:
            ...
            # Add regularization
            disc_reg = self.disc_regularizer(logit_real, real_batch, logit_fake, gen_batch)
        ...
        with tf.GradientTape() as tape:
            ...
            mmd_loss = self.mmd_loss(
                res_gen, res_real,
                self.mmd_kernel, self.mmd_kernel_widths,            
    ...
    #        print("\nStart step", step)
            ...
            d_loss, g_loss = self.train_step(real_batch, mass_batch)

            # Logging.
            if step % (iterations//10) == 0:
                # Print metrics
                print("Epoch #{}: Generative Loss: {}, Discriminator Loss: {}, Learning Rate: {}"
                    .format(step, g_loss, d_loss, g_optimizer._decayed_lr(tf.float32)))
...
# Settings
max_iteration = 200
step_factor =  int(max_iteration/10)
batch_size = 100
latent_dim = 2
embedding = True
gamma = 0.001
...
# define the networks
generator = define_generator(latent_dim, n_outputs=1)
discriminator = define_discriminator(n_inputs=1, embedding=embedding)



def train_gan(iterations=2000):
    save_dir = "results"
...
#        print("\nStart step", step)
        ...
        # Train the discriminator & generator on one batch of real images.
        d_loss, g_loss, generated_images = train_step(real_batch)
